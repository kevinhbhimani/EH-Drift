/* program to evolve e and h charge distributions from surface-alpha interactions
   ...
   This version is specifically for alpha interactions at r = 8 mm 
*/
...
int main(int argc, char **argv)
{
  ...
  for (i = 0; i < LL; i++) {
    for (j = 0; j < R; j++) {
      ...
      rho_e[3][i][j] = setup.impurity[i][j]; // save impurity values
    }
  }
  ...
  /* read starting rho_e values */
  if (0 &&
      (read_rho(LL, R, grid, rho_e[0], "start_ed.dat") ||
  ...
  */
  ...
  for (int n=1; n<=3000; n++) {
    ...
    if (n%10 == 0) {
      char fn[256];
      sprintf(fn, "ed%3.3d.dat", n/10);
      if (esum2 > 0.1) write_rho(LL, R, grid, rho_e[0], fn);
      sprintf(fn, "hd%3.3d.dat", n/10);
