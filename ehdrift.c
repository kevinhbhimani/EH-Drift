   ...
   Unknowns/guesses:
     ...
     - Effective passivated-surface thickness
         ...
         ~ But surface roughness and damage from the passivation process could be
           2-10 μm thick
  Can be run as ./ehdrift config_files/P42575A.config -a 15.00 -z 0.05 -g P42575A -s 0.00
  WP can be calculated as ./ehdrift config_files/P42575A_calc_wp.config -a 15.00 -z 0.05 -g P42575A
*/
  ...
  for (n=1; n<=4000; n++) {   // CHANGEME : 4000 time steps of size time_steps_calc (0.02) thus simulating 800ns
    ...
    for (z=1; z<LL; z++) {
      for (r=1; r<R; r++) {
        ...
        // adds the impurities generated by charge cloud to detector impurities
        // note that intial detector impurity are not changed, and we can add impurities since they are in form of electrons
    ...
    printf("Attempting to call the GPU function\n");
    cuda_hello_world();
    break;
    ...
    if (n%10 == 0) {
      char fn[256];
      sprintf(fn, "siggen_sims_data/%s/q=%.2f/drift_data_r=%.2f_z=%.2f/ed%3.3d.dat",det_name,setup.impurity_surface,alpha_r_mm,alpha_z_mm, n/10);
      if (esum2 > 0.1) write_rho(LL, R, grid, rho_e[0], fn);
      sprintf(fn, "siggen_sims_data/%s/q=%.2f/drift_data_r=%.2f_z=%.2f/hd%3.3d.dat",det_name,setup.impurity_surface,alpha_r_mm,alpha_z_mm, n/10);
