...
int main(int argc, char **argv)
{
  ...
  */
  ...
  for (int n=1; n<=4000; n++) {   // CHANGEME : 4000 time steps of size time_steps_calc (0.02) thus simulating 800ns
    printf("\n\n -=-=-=-=-=-=-=-=-=-=-=- n = %3d  -=-=-=-=-=-=-=-=-=-=-=-\n\n", n);
    ...
    if (n%10 == 0) {
      char fn[256];
      sprintf(fn, "/global/cscratch1/sd/kbhimani/siggen_sims/%s/q=%.2f/drift_data_r=%.2f_z=%.2f/ed%3.3d.dat",det_name,setup.impurity_surface,alpha_r_mm,alpha_z_mm, n/10);
      if (esum2 > 0.1) write_rho(LL, R, grid, rho_e[0], fn);
      sprintf(fn, "/global/cscratch1/sd/kbhimani/siggen_sims/%s/q=%.2f/drift_data_r=%.2f_z=%.2f/hd%3.3d.dat",det_name,setup.impurity_surface,alpha_r_mm,alpha_z_mm, n/10);
