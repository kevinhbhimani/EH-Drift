...
def make_png(fname1):
    ...
    file_load = '/pscratch/sd/k/kbhimani/siggen_ccd_data/5000.00_keV/grid_0.0200/self_repulsion_1/P42575A/q=0.00/drift_data_r=15.00_z=0.10/'
    file_save = '/pscratch/sd/k/kbhimani/siggen_ccd_data/gif_data/r_15_z_0p10/grid=20_micron/proj_z/eh'
    ...
    ax1.set_title("Electron/Hole density r=15, z=0.10, grid=20 micron \n", fontsize=16, linespacing=0.3)
    ...
    xx = np.arange(len(y))
    xx = xx * max(y)/len(y)
    zz1 = zvals.sum(axis=1)
    zz2 = zvals2.sum(axis=1)
    ax3.semilogy(xx, zz1, '-')
    ax3.semilogy(xx, zz2, '-')
    ax3.set_xlim(0, 0.75)
    ax3.set_xlabel("Height [mm]", size=13)
